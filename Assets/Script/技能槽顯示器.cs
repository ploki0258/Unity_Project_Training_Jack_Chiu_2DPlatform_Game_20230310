...
public class 技能槽顯示器 : MonoBehaviour
{
	...
	void 刷新內容()
	{
		...
		if (zXCType == ZXCType.X)
		{
			// 欄位標示.文字 = X
			textSlot.text = "X";
			//  如果 存檔管理器.玩家資料.技能槽X的技能ID 不等於 -1的話
			if (SaveManager.instance.playerData.skillX != -1)
			{
				// 依據技能槽X的技能ID回傳技能資料並儲存為skill
		...
		if (zXCType == ZXCType.C)
		{
			// 欄位標示.文字 = C
			textSlot.text = "C";
			//  如果 存檔管理器.玩家資料.技能槽C的技能ID 不等於 -1的話
			if (SaveManager.instance.playerData.skillC != -1)
			{
				// 依據技能槽C的技能ID回傳技能資料並儲存為skill
